...
The stream is cancellable mid-flight: `response.chunk()` races a `tokio::sync::Notify` (per `request_id`, in a `CancelRegistry` Tauri state) via `tokio::select!`. `ollama_cancel(request_id)` latches an `AtomicBool` and calls `notify_waiters()`, so Rust stops draining Ollama immediately instead of burning quota after Stop. A `CancelGuard` removes the registry entry on every exit path. On cancel, `ollama://done` fires with the partial content so far, and `streamChat`'s `abort` listener invokes `ollama_cancel`. A pre-cancel tombstone handles the race where Stop fires before `register()` runs; `requestId` is nulled after `done`/`error` resolve so a late Stop doesn't insert a tombstone for an already-finished request. Cancellation is checked at the top of the round loop and after each round resolves, so a run stopped mid-tool-round still finishes that round's in-flight calls (including network ones) before it ends.

A run is stopped by **replacing the pane it streams into**, which is a second abort path distinct from Stop: `loadMessages`/`clearMessages` call `abortController.abort()`, and both reset `isStreaming`/`abortController` (`clearMessages` did not, which is why New Chat mid-run used to leave a spinner that could never be cleared). Without it, switching session or starting a new chat mid-run left Rust draining Ollama and the tool loop still spending calls on an answer nothing could display. The matching frontend check is `paneIsCurrent()` in `useStreamingChat`: it captures the store's `chatNonce` at send time — `chatNonce` moves only on `loadMessages`/`clearMessages`, i.e. exactly "this pane was handed another conversation" — and `onDone` and the error path `return` early when it has. That guard is what makes an orphaned run harmless: `saveOnReply`/`saveNow` read `store.getState().messages` at **save** time but write to the session id captured at **send** time, so a run that outlived the switch wrote the new session's messages into the old session's row (`sync_messages` upserts by id and deletes the ones that are absent, so switching replaced A's conversation with B's, and New Chat reached the empty-list branch that erased A's rows outright). The aborted run's partial output is deliberately dropped rather than saved — `loadMessages` already replaced its message, so there is nothing left to save it from. Side chats are unaffected: a tab's store is never reloaded on a session switch, and its run keeps writing to its own parent session.
...
- **Reset a chat's per-pane state** — bump `chatNonce` (`clearMessages`/`loadMessages` already do). Per-chat-default effects should watch `chatNonce`, not `model`. Bumping it also aborts whatever run is in flight, because it is the pane's identity for that run (see the streaming section).
...
- **Every `ChatPane` must be given `sessionId`** (`App.jsx` for the main pane, `SidePanel.jsx` for side-chat tabs). Attaching and detaching write the root set to the *session*, and that write is guarded on this prop — the main pane was rendered without it, so folders attached there reached the pane store and nothing else, and vanished on the next session load. It failed silently, because `db.setProjectRoots` swallows its own rejection (`.catch(noop)`). `src/App.test.jsx` pins the wiring; `ChatPane.test.jsx` covers attach and detach persisting the whole list. A pane with no session is legitimate (a brand-new chat): `createSession` writes the pane's roots when the row is created.
