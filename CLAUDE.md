...
- **After bumping `package.json`'s version, run `npm install` (not just hand-edit) so `package-lock.json`'s own two `version` fields (top-level and the `""` root package) get updated too.** Hand-editing only `package.json` leaves the lockfile's version stale; `npm ci` in the release workflow then fails with `EUSAGE`/"can only install packages when your package.json and package-lock.json ... are in sync" — this broke the v1.1.0 release run and required a re-tag. Always run `npm ci` locally after a version bump to confirm the lockfile is actually in sync before tagging.
