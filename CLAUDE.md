...
**All markdown element styling lives in the `components` prop of `<ReactMarkdown>` in `MarkdownBody.jsx` (inline styles), not in CSS.** The `.markdown-body …` block in `index.css` (headings, `strong`, `a`, `blockquote`, `table`) is **dead**: the component applies the CSS-module class `styles.markdownBody` (a hashed name), so the literal global `.markdown-body` selector never matches. Elements without a `components` override (previously the headings, and `table`/`th`/`td`) fall back to browser defaults — which is why h1/h2/h3 were oversized until `h1`/`h2`/`h3` overrides were added, and why tables rendered squished (auto column sizing shrank a column to near-zero, and the bubble's inherited `word-break: break-word` then split words letter-by-letter to fit). The fix: `table` uses `table-layout: fixed` + `width: 100%` so columns split evenly within the bubble, and `th`/`td` override back to `word-break: normal` / `overflow-wrap: break-word`. `theme.js` already had `mdThBg`/`mdThText`/`mdTdBorder` tokens defined for this — they were dead until the `table`/`th`/`td` overrides were added. `overflow-wrap: normal` (rather than `break-word`) was tried first but reintroduced a different bug: with fixed equal-width columns, a cell whose content doesn't fit its column and has no wrap point right at the boundary just overflows past the cell into the neighboring column's text instead of wrapping, visually overlapping the two. `break-word` only breaks a word as a last resort (unlike `word-break: break-word`, which splits letter-by-letter), so it wraps within the column without the squished look the original bug had. To style a markdown tag, add/edit its entry in the `components` prop in `MarkdownBody.jsx`; don't edit the dead `.markdown-body` rules.
...
- **Branch a side chat from another side chat** — side chats can nest: a side chat's `parentSideChatId` (`sessionStore.addSideChat(sessionId, model, parentSideChatId)`) marks it as branched from another side chat instead of the main chat. Two entry points create one: the `GitBranch` icon in `ChatPane`'s header (next to delete, `onBranch` prop wired in `SidePanel.jsx`), and "Ask in side chat" from a text selection *inside* a side chat (`MessageBubble.jsx`'s `parentSideChatId` prop, passed by `ChatPane` as `sideChatId` when `isSideChat`). The child's `contextStore` (what `useStreamingChat` injects as "recent conversation" context) resolves to `getSideChatStore(parentSideChatId)` instead of `useMainChat` — see the ternary in `SidePanel.jsx`'s `ChatPane` render. Tab labels are computed by `buildSideChatTree()` in `SidePanel.jsx`: a depth-first walk of the parent/child relationships assigns path labels (`1`, `1.1`, `1.1.1`, ...) *and* the tab display order in one pass, so a child tab always renders immediately after its parent regardless of creation order across the flat `sideChats` array. The tab bar itself is rendered as stacked rows, not a single flat row: `buildTabRows()` walks from the active tab up to the root and emits one row per level — the root row, then each ancestor's children row, ending with the active tab's own children (if any) — so a branch's children only appear in the tab bar while you're inside that branch (active tab is it or a descendant); switching away collapses the row instead of leaving every branch's children permanently listed. `TabButton`'s tooltip (the tab number's hover text) reads its content — the first user question, falling back to "Branched from X" for an empty chat — via `getSideChatStore(sc.id)((s) => s.messages...)`, the same live per-tab zustand store used for the `isStreaming` dot, rather than the `sc.messages` field on the `sessionStore` session object; the latter is a write-through snapshot that can lag behind the live store (e.g. mid-stream, before a save round-trips), so any per-tab UI derived from message content should read the live store the same way.
...
- **After bumping `package.json`'s version, run `npm install` (not just hand-edit) so `package-lock.json`'s own two `version` fields (top-level and the `""` root package) get updated too** — hand-editing only `package.json` leaves the lockfile's version stale.
- **This machine's `~/.npmrc` sets `legacy-peer-deps=true` globally, which makes local `npm install`/`npm ci` silently tolerant of a lockfile missing optional-peer entries (e.g. `@types/react`, `@types/prop-types`, `csstype`, pulled in as `@testing-library/react`'s optional peer deps) — CI has no such config and enforces strict validation, so a lockfile that installs fine locally can still fail `npm ci` in CI with `EUSAGE`/"Missing: X from lock file".** This caused two consecutive v1.1.0 release-workflow failures even after the lockfile *looked* back in sync locally. To actually reproduce CI's behavior before tagging, verify with `rm -rf node_modules && npm ci --no-legacy-peer-deps` (not plain `npm ci`, which inherits the permissive `~/.npmrc` default) — and regenerate the lockfile with `npm install --no-legacy-peer-deps` if it fails.
