...
export function useDbInit() {
  const { setSessionsFromDb, hydrateSession, setActiveChatId } = useSessionStore()
  const loadMessages = useMainChat(s => s.loadMessages)

  useEffect(() => {
    db.loadSessions().then(async sessions => {
      if (!sessions || sessions.length === 0) return
      setSessionsFromDb(sessions)
      const last = sessions[0] // already ordered by created_at DESC
