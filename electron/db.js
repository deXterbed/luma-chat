...
function init() {
  ...
  for (const table of ["messages", "side_chat_messages"]) {
    try {
      db.exec(
        `ALTER TABLE ${table} ADD COLUMN tool_calls TEXT NOT NULL DEFAULT '[]'`,
      );
    } catch {}
  }
}
...
function loadSessionMessages(sessionId) {
  ...
  const sideChats = db
    ...
    .map((sc) => {
      const scMessages = db
        .prepare(
          "SELECT * FROM side_chat_messages WHERE side_chat_id = ? ORDER BY position",
        )
        .all(sc.id)
        .map((m) => ({
          id: m.id,
...
const insertMessage = db_lazy(() =>
  db.prepare(
    "INSERT OR REPLACE INTO messages (id, session_id, role, content, images, tool_calls, position) VALUES (@id, @session_id, @role, @content, @images, @tool_calls, @position)",
  ),
);
...
const deleteAllSideChatMessages = db_lazy(() =>
  db.prepare("DELETE FROM side_chat_messages WHERE side_chat_id = ?"),
);
const insertSideChatMessage = db_lazy(() =>
  db.prepare(
    "INSERT OR REPLACE INTO side_chat_messages (id, side_chat_id, role, content, images, tool_calls, position) VALUES (@id, @side_chat_id, @role, @content, @images, @tool_calls, @position)",
  ),
);
...
function setActiveSideChat(sessionId, sideChatId) {
  db.prepare("UPDATE side_chats SET is_active = 0 WHERE session_id = ?").run(
    sessionId,
  );
  if (sideChatId) {
    db.prepare("UPDATE side_chats SET is_active = 1 WHERE id = ?").run(
      sideChatId,
    );
  }
}
